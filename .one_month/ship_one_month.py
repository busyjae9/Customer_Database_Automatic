...
df_two_month_ago = cn.pd.DataFrame(
    cn.Worksheet.order_two_months_ago.get_all_records())
df_tri_month_ago = cn.pd.DataFrame(
    cn.Worksheet.order_tri_months_ago.get_all_records())
...
def select_ship_one(df):
    ...
    df_two_ago_blank = df_two_month_ago[df_two_month_ago['비고'] == '']
    ...
    for index, row in df.iterrows():
        ...
        # for i in list(set(df_one_ago_blank['Address'])):
...
def select_ship_two(df_all_ship):
    ...
    df_ship_id_match = cn.pd.DataFrame()
    df_ship_email_match = cn.pd.DataFrame()
    df_tri_ago_blank = df_tri_month_ago[df_tri_month_ago['비고'] == '']
    # df_ship_address_match = cn.pd.DataFrame()

    for index, row in df_all_ship.iterrows():
        row['Ship'] = row['Ship'] + 'X'
        df_not_ship = df_not_ship.append(row)
        for i in list(set(df_tri_ago_blank['Reference Txn ID'])):
            if row['Reference Txn ID'] == '':
                pass
            elif row['Reference Txn ID'] == i:
                ...
                df_ship_id_match = df_ship_id_match.append(row)
        for i in list(set(df_tri_ago_blank['From Email Address'])):
            if row['From Email Address'] == i:
                if row['Ship'][len(row['Ship']) - 1] == 'O':
                    pass
                else:
                    row['Ship'] = replaceRight(row['Ship'], 'X', 'O', 1)
                df_ship_email_match = df_ship_email_match.append(row)
        # for i in list(set(df_tri_ago_blank['Shipping Address'])):
        #     if row['Shipping Address'] == i:
