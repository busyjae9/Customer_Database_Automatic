...
def box_select(df):
    ...
    for index, row in df.iterrows():
        if row['Subject'].lower().find('kpopbox') >= 0:
    ...
    etc_people = etc_people.drop_duplicates(
        ['Reference Txn ID', 'From Email Address', 'Shipping Address', 'Subject', 'Date', 'Name'], keep=False)
...
def preference(df):
    ...
    df_pre = df_pre[[
        ...
        'From Email Address', 'Shipping Address', 'Subject', 'New', 'Ship', 'Preference']]
