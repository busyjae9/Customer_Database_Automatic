...
def save_data_at_cus_2(df):
    ...
    cus_2 = cus_2.drop_duplicates(['Date', 'Transaction ID', 'Email', 'Name'], keep = 'last')
...
def save_data(row, temp_df, df_cus):
...
def order_check(row, df):
    if row['Reference Txn ID'] == '':
        ...
        number = df.loc[idx[0], '비고'].split()[-1]
        if number.isdigit():
            number = (lambda x : int(x) + 1 if '라벨 재출력됨' in df.loc[idx[0], '비고'] else x)(number)
            df.loc[idx[0], '비고'] = df.loc[idx[0], '비고'].replace(df.loc[idx[0], '비고'][-1], str(number))
        else:
            df.loc[idx[0], '비고'] =  df.loc[idx[0], '비고'] + ' 라벨 재출력됨 1'
        print(df.loc[idx[0], '비고'])
...
def data_finish(df):
    ...
    for index, row in df.iterrows():
        if (row['Ship'] == "labelled") & (row['Confirm'] != ''):
            if df_cus[(df_cus['Transaction ID'] == row['Transaction ID']) & (df_cus['Name'] == row['Name'])].empty == False :
                same_idx = df_cus[(df_cus['Transaction ID'] == row['Transaction ID']) & (df_cus['Email'] == row['From Email Address'])].index
                df_cus = df_cus.drop(same_idx)
    for index, row in df.iterrows():
        if (row['Ship'] == "labelled") & (row['Confirm'] != ''):
            temp_df_cus = temp_df_cus.append(save_data(row, temp_df_cus, df_cus))
...
def stock_func(i_txt, df, row):
    ...
    elif i_txt.lower().find('bi-monthly') >= 0 :
...
if __name__ == "__main__":
    ...
    while(True):
        name = input("어떤 시트의 라벨을 다시 뽑을까요? : ")
        
        try:
            select = cn.doc_select.worksheet(name)
            break
        except:
            print("{} 이름의 시트가 없습니다. 다시 입력바랍니다.\n".format(name))
            
    df_select = cn.pd.DataFrame(select.get_all_records())
    print(df_select.columns)
    save_label(df_select)
    stock_notice(df_select)
    data_finish(df_select)