...
# 케이팝 박스 구분
SET_KEY = ['Date', 'Type', 'Transaction ID', 'Contact Phone Number', 'Reference Txn ID', 'Name', 'From Email Address', 'Shipping Address', 'Address 1', 'Item Title', 'New', 'Ship', 'Shipping Courier']
set_pre = ['Date', 'Type', 'Transaction ID', 'Contact Phone Number', 'Reference Txn ID', 'Name', 'From Email Address', 'Shipping Address', 'Address 1', 'Item Title', 'New', 'Ship', 'Shipping Courier', 'Preference']
...
def box_loop_num(df):
    ...
    temp1 = loop_txt.loc['1 Loop', 'contents']
    item_set_loop_one = temp1.replace(" ", "").split(",")
    temp2 = loop_txt.loc['2 Loop', 'contents']
    item_set_loop_two = temp2.replace(" ", "").split(",")
    check_group = loop_txt.loc['추가 Group', 'contents'].replace(" ", "").split(",")
    
    # item_set_loop_one = ["저금통", "파우치", "족자", "명찰"]
    # item_set_loop_two = ["스포츠타올", "씨디케이스", "필통", "손수건", "명찰"]
...
def box_loop_alpha(df, second = False):
    ...
    tempA = loop_txt.loc['A Loop', 'contents']
    item_set_loop_A = tempA.replace(" ", "").split(",")
    tempB = loop_txt.loc['B Loop', 'contents']
    ...
    if second == False :
        ...
        for index, row in df.iterrows():
            ...
            if (row['Loop_Alpha'] == 'Loop_A') & ((row['Duplicated_Alpha'].find('D') >= 0) or (row['Duplicated_Alpha'].find('1') >= 0)):
                for i in item_set_loop_B:
                    product_list = []
                    for j in temp_line:
                        product = j + '_' + i
