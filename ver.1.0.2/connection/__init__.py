...
class Time :
    ...
    # now_str_ = datetime.date.strftime(now_, "%Y%m")
    # while(True):
    #     now = input("이번달은 '{}' 입니다.\n원하는 년과 달을 입력해주세요(예시 : 202106) : ".format(now_str_))
    #     try:
    #         now_datetime = datetime.datetime.strptime(now, "%Y%m")
    #         break
    #     except:
    #         print("오류입니다. 다시 입력바랍니다.\n")
    
    now = '202106' 
    now_datetime = datetime.datetime.strptime(now, "%Y%m")

    now_str = datetime.date.strftime(now_datetime, "%Y%m")
    now_time = datetime.date.strftime(now_datetime, "%m월 %d일 라벨 작업 완료")
    one_month_ago = now_datetime - relativedelta(months=1)
    one_month_ago_str = datetime.date.strftime(one_month_ago, "%Y%m")
    two_months_ago = now_datetime - relativedelta(months=2)
    two_months_ago_str = datetime.date.strftime(two_months_ago, "%Y%m")
    tri_months_ago = now_datetime - relativedelta(months=3)
    tri_months_ago_str = datetime.date.strftime(tri_months_ago, "%Y%m")
    
    months_list = [now_str, one_month_ago_str, two_months_ago_str, tri_months_ago_str]
    print(months_list)
...
class Worksheet :
    ...
    order_sheetlist = [order, order_one_month_ago, order_two_months_ago, order_tri_months_ago]

    # 고객 데이터의 총 누적 시트
    cus_1 = doc_cus.worksheet('all_data_1')
    cus_2 = doc_cus.worksheet('all_data_2')
    cd_cus = doc_cus.worksheet('kpopcd preference')

    # 재고 데이터
    stock = doc_stock.worksheet("ultimate")
    box_loop = doc_stock.worksheet("BOXLOOP")
    etc_box = doc_stock.worksheet("etc_box")
    box_price = doc_stock.worksheet("box price")
    # 라벨 데이터
    # label = doc_label.worksheet(Time.now_str)

    select_test = doc_select.worksheet('test')
...
class Dataframes :
    ...
    etc_box = pd.DataFrame(Worksheet.etc_box.get_all_records())
    box_price = pd.DataFrame(Worksheet.box_price.get_all_records())
    cus_1 = pd.DataFrame(Worksheet.cus_1.get_all_records())
    cus_2 = pd.DataFrame(Worksheet.cus_2.get_all_records())
    cus = cus_1.append(cus_2)
    cd_cus = pd.DataFrame(Worksheet.cd_cus.get_all_records())
    select = pd.DataFrame(Worksheet.select.get_all_records())
    select_test = pd.DataFrame(Worksheet.select_test.get_all_records())
    ...
    order_list = [order, order_one_month, order_two_months, order_tri_months]
